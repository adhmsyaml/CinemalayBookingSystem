...
if(isset($_GET['id'])){
	$mov = $conn->query("SELECT * FROM theater_settings where ts_id =".$_GET['id']);
	...
	<div class="col-lg-12">
		<form id="manage-seat">
			<div class="form-group">
				<input type="hidden" name="id" value="<?php echo isset($_GET['ts_id']) ? $_GET['ts_id'] : '' ?>">
				...
				<select name="theater_id" required="" class="custom-select browser-default" >
					...
					?>
					<option value="<?php echo $row['theater_id'] ?>" <?php echo isset($meta['theater_id']) && $meta['theater_id'] == $row['theater_id'] ? 'selected' :'' ?>><?php echo $row['name'] ?></option>
				<?php endwhile; ?>
				</select>
			</div>
