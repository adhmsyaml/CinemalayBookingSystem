...
while($row=$ts->fetch_assoc()){
	$data[] = $row;
	$seat_group[$row['ts_id']] = $row['seat_group'];
	$seat_count[$row['ts_id']] = $row['seat_count'];
}

 $mov = $conn->query("SELECT * FROM movies where movie_id =".$_GET['mid'])->fetch_array();
...
<div class="row">
	...
	<label for=""class="control-label">Pick Type</label>
      <select id="movie" class="custom-select browser-default">
        <option value="10">student price: RM10</option>
        <option value="12">Classic: RM12</option>
        <option value="8">Kids: RM8</option>
      </select>
	 </div>
	...
	<div class="form-group col-md-8">
		<label for="" class="control-label">Payment can be made below</label>
			<?php 
			echo ' 
			<html>
			<img src="assets/img/pay.jpeg" width="400" height="400" />
