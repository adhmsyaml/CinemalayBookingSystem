...
Class Action {
	...
	function save_movie(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("UPDATE movies set ". $data." where movie_id =".$id);
			if($save)
				return 1;
		}
	}
	function delete_movie(){
		extract($_POST);
		$delete  = $this->db->query("DELETE FROM movies where movie_id =".$id);
		if($delete)
			return 1;
	}
	function delete_theater(){
		extract($_POST);
		$delete  = $this->db->query("DELETE FROM theater where theater_id =".$id);
		if($delete)
			return 1;
	}
	function save_theater(){
		...
		$save = $this->db->query("UPDATE theater set name = '".$name."' where theater_id = ".$id);
		if($save)
			return 1;
	}
	function save_seat(){
		...
		$save = $this->db->query("UPDATE theater_settings set ".$data." where ts_id = ".$id);
		if($save)
			return 1;
	...
	function delete_seat(){
		extract($_POST);
		$delete  = $this->db->query("DELETE FROM theater_settings where ts_id =".$id);
		if($delete)
			return 1;
	}
